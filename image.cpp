...
namespace Image {
	...
	uint8_t getPixelAlt(const uint8_t image[HEIGHT*WIDTH], int y, int x) {
	...
	uint8_t getPixel(const uint8_t image[HEIGHT][WIDTH], int y, int x) {
		...
		for (int i = y-1; i <= y+1; i++)
			for (int j = x-1; j <= x+1; j++)
				sum += getPixelAlt(image, y, x);
		return sum / 9;
	}
	uint8_t getPixel(const uint8_t image[HEIGHT][WIDTH], std::pair<int, int> yx) { return getPixel(image, yx.first, yx.second); }
	uint8_t getPixel(const uint8_t image[SIZE], int y, int x) { return getPixel((const uint8_t (*)[WIDTH])image, y, x); }
	uint8_t getPixel(const uint8_t image[SIZE], std::pair<int, int> yx) { return getPixel((const uint8_t (*)[WIDTH])image, yx.first, yx.second); }
